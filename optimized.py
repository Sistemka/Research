...
def optimized_output(input_tensor, next_step_accuracy_percent = 0.5, next_step_point = 5, comparing_percent = 0.8): #next_step_point это минимальное количество фоток для следующего шага
    matching_items = []
    for label in all_labels:
        currently_accepted_items = []
        comparing_list = []
        for file in os.listdir(os.path.join(base_dir, label)): #SELECT * from all_tensors WHERE "type" == label
            file_path = os.path.join(base_dir, label, file) # GET url field 
            comparing_tensor = np.expand_dims(img2vec_that_saves_proportions(file_path), 0) #change img2vec if needed
